...
// static MY_STATIC: OnceCell<i32> = OnceCell::new();

// use lazy_static::lazy_static;

// lazy_static! {
//     static ref NUMBER_MAP: Vec<Vec<usize>> = initialize_number_map(),
//     static ref TOTAL_MAP: [usize; 512] = initialize_total_map(),
//     static ref INVERSE_MAP: HashMap<usize, usize> = initialize_inverse_map();
// }
...
impl Grid {
    ...
    fn output(&self) -> String {
        self.cells.iter().map(|v| ((INVERSE_MAP[*v] >> 1) + 1).to_string()).collect::<Vec<_>>().join("")
    }
...
fn main() {
    ...
    socket_wrapper::run(String::from("rust:optimised"), |input| {
        solve(&input)
    })
}
...
fn solve(input: &str) -> String {
    ...
    for (i, char) in input.chars().enumerate() {
        if char != '0' {
            cells[i] = 1 << (char.to_digit(10).unwrap() as u8) - 1;
        }
    }
