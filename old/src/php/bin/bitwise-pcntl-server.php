...
// global $totalMap;
// global $possibleNumberMap;
\Dolondro\Sudoku\NetworkSolver::handle("php:bitwise-pcntl-server", function($request) use ($totalMap, $map, $possibleNumberMap){
    $rows = explode(" ", trim(chunk_split($request, 9, " ")));
    ...
    foreach ($rows as $row) {
        for ($x=0; $x<9; ++$x) {
            $value = $row[$x] ?? "0";
            $cells[] = $value !== "0" ? toBinary($value) : null;
        }
    }
